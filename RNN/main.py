from RNN.LSTM_LM import LSTM_LangModel
...
def main():
    ...
    model_path = "RNN/saved_models/"
    with open(data_path + "greeklish/greeklish_europarl_test_5k.txt", "r", encoding="utf-8") as file:
        test_sources = []
        for line in file:
    ...
    #     torch.load(model_path + "LSTM_LM_50000_char_120_32_512.pt", map_location=torch.device('cpu')))

    text_vec = TextVectorizer("char")

    with open(data_path + "greek/greek_europarl_training_100k.txt", "r", encoding="utf-8") as file:
        data = []
        for line in file:
            data.append(line)
    ...
    text = "Uparxoun autoi pou kerdizoun apo mia katastash kai autoi pou xanoun"
    # Number of beams does matter.
    results = LM.translate([text], 5)
    print(results)
    
    
    print("\n")

    with open(data_path + "greek/greek_europarl_test_5k.txt", "r", encoding="utf-8") as file:
        test_targets = []
        for line in file:
